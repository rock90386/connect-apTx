...
namespace ns3 {
  ...
  //
    ...
    Ptr<PcapFileWrapper> file2 = pcapHelper.CreateFile ("fd-create-pcap2.pcap", std::ios::out, PcapHelper::DLT_EN10MB);
    Ptr<PcapFileWrapper> file3 = pcapHelper.CreateFile ("fd-create-pcap3.pcap", std::ios::out, PcapHelper::DLT_EN10MB);
    if (promiscuous)
    {
      pcapHelper.HookDefaultSink<FdNetDevice> (device, "PromiscSniffer", file,file2,file3);
    }
    else
    {
      pcapHelper.HookDefaultSink<FdNetDevice> (device, "Sniffer", file, file2,file3);
    }
  }
